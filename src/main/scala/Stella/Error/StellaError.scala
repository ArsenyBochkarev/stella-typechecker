...
enum StellaError(errStr: String = "Unknown error"):
  ...
  case ERROR_UNEXPECTED_TYPE_FOR_EXPRESSION(expr: String, exprType: String, expectedType: String) extends StellaError(
    s"Unexpected type for expression ${expr}: got ${exprType}, expected ${expectedType}")
  case ERROR_NOT_A_FUNCTION(expr: String) extends StellaError(s"Not a function: ${expr}")
//  case ERROR_NOT_A_TUPLE
...
//  case ERROR_NOT_A_LIST
  ...
  s"Expected function type ${expectedType} for expression ${expr} but got: ${exprType}")
  case ERROR_UNEXPECTED_TYPE_FOR_PARAMETER(expr: String, exprType: String, expectedType: String) extends StellaError(
    s"Unexpected type for parameter ${expr}: got ${exprType}, expected ${expectedType}")
  case ERROR_UNEXPECTED_TUPLE extends StellaError()
  case ERROR_UNEXPECTED_RECORD extends StellaError()
  ...
  case ERROR_TUPLE_INDEX_OUT_OF_BOUNDS extends StellaError()
